  ...
  .get(async (request, response) => {
    try {
      ...
      return response.json({
        error: `Error, couldn't find a application given the following id`
      });
    }
  })
    ...
    Application.findByIdAndUpdate(
      ...
      (err, model) => {
        if (!err) {
          ...
          return response.json({
            error: `Error, couldn't update a application given the following data`
          });
        }
      }
    );
  ...
  .delete((request, response) => {
    Application.findByIdAndDelete(request.params.id, (err, model) => {
      if (!err) {
        ...
        return response.json({
          error: `Error, couldn't delete a application given the following data`
        });
      }
    });
  });
