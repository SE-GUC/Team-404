...
const functions = {
    ...
    getSpecificEvent: async () => {
      const event = await axios.get('http://localhost:3001/Routes/api/events/5cb233a3cef46afa8f9bd0f5')
      return event
  },
    updateEvent: async (_id) => {
        const event = await axios.put('http://localhost:3001/Routes/api/events/5cb23616cef46afa8f9bd107', {
            eventName: 'java'
        });
        return event;
    },
    deleteEvent: async (_id) => {
      const res = await axios.delete('http://localhost:3001/Routes/api/events/5cb85dd38dc4c45764b0b8bf');
      return res;
  },
    createEvent: async () => {
